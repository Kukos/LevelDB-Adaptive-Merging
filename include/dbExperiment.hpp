...
class DBExperiment{
    ...
    public:
        ...
        static void expDiffDataSize() noexcept(true);

        static void expDiffThread() noexcept(true);

        static void experimentNoModification(std::string expType, std::ofstream& log, const size_t dataRange, const size_t nmbRec, const size_t nmbQuery, const size_t sel, const size_t thread=80) noexcept(true);
